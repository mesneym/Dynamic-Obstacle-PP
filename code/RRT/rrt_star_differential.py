...
class Node:
    def __init__(self, state=None, cost=0.0, costToCome=0.0, parent=None, vel=[0.0, 0.0, 0.0]):
        self.state = state
        self.parent = parent
        self.cost = cost
...
def steer(xNearest, xRand):
    ...
    if dist < stepsize:
        ...
        t = stepsize / dist
        v = xRand[0:2] - xNearest[0:2]
        r = t * v + xNearest[0:2]
        return np.array([r[0], r[1], xNearest[2]])
...
def findNeighbors(NodesExplored, radiusClearance, newNode, radius=2.0):
    neighbours = []
    newX, newY, _ = newNode.state
    xBest = newNode

    for key, node in NodesExplored.items():
        posX, posY, _ = node.state
        if (node.state[0:2] != newNode.state[0:2]).all() and isObstacleFree(node.state, newNode.state, radiusClearance):
...
def generatePath(q, startEndCoor, nodesExplored, robotParams, dt, radiusClearance):
    # get start and goal locations
    sx, sy, st = startEndCoor[0]
    gx, gy, gt = startEndCoor[1]

    # Initializing root node
    key = str(sx) + str(sy)
    root = Node(np.float32([sx, sy, st]), 0.0, 0.0, None, [0.0, 0.0, 0.0])
    ...
    while True:
        ...
        xRand = np.array([newPosX, newPosY, 0])
        ...
        newPosX, newPosY, newOrientation, x_dot, y_dot, omega = constraints(x, y, t, 0, robotParams[0],
                                                                            robotParams, dt)
        nearestCoor.append(np.array([newPosX, newPosY, newOrientation, x_dot, y_dot, omega]))
        nearestDist.append(distance(np.array([newPosX, newPosY, newOrientation]), xNew))
        ...
        xNew = nearestCoor[nearestDist.index(min(nearestDist))][0:3]
        xNewVelocities = nearestCoor[nearestDist.index(min(nearestDist))][3:6].tolist()
        ...
        newNode = Node(xNew, xNewCost, xNewcostToCome, nodesExplored[xNearestKey], xNewVelocities)
        ...
        if distance(newNode.state, [gx, gy, gt]) <= 0.3:
...
if __name__ == "__main__":
    ...
    if not isSafe(startCoor, res, clearance + robotRadius) or not isSafe(goalCoor, res, clearance + robotRadius):
        ...
        success, solution = generatePath(q, startEndCoor, nodesExplored, robotParams, dt, clearance + robotRadius)
        endTime = time.time()
